...
function ImgButton:draw()
    ...
    if hover and Gamestate.current() ~= GS.WARNINGWIN then
        self.color.l = 50
        self.color.a = 90
    ...
    Color.set(self.color)
    local img = self.img
    if hover and self.hover_img and Gamestate.current() ~= GS.WARNINGWIN then img = self.hover_img end
    love.graphics.draw(img, self.pos.x, self.pos.y, 0, self.scale, self.scale)

    if hover and self.hover_text and Gamestate.current() ~= GS.WARNINGWIN then
